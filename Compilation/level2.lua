...
-- Larger images
	space = display.newImage("space.png")
	screenGroup:insert( space )
	

	
	space2 = display.newImage("space.png")
	--space2.y = 315
	space2.x = 600
	--space2.alpha = 0.75
	screenGroup:insert( space2 )
	
	space3 = display.newImage("space.png")
	--space3.y = 315
	space3.x = 400
	--space3.alpha = 0.75
	screenGroup:insert( space3 )
	
	space4 = display.newImage("space.png")
	--space3.y = 315
	space4.x = 0
	--space4.alpha = 0.75
	screenGroup:insert( space4 )
	
	
	emitterTarget = display.newImage( "emitter.png" )
	emitterTarget.x = 760
	emitterTarget.y = 300
	emitterTarget.name = "emitterTarget"
	emitterTarget.alpha = 0
	screenGroup:insert( emitterTarget )	
...
yAxisRandomBeam = math.random (20, 180)
	...
	beam[1] = display.newImage( myImageSheet , sheetInfo:getFrameIndex("beam2"))
	beam[1].x = 760
	beam[1].y = 100
	beam[1].name = "beam"
	physics.addBody( beam[1], dynamic, { density=1, friction=0, bounce=0.0, radius=beam[1].radius, filter=jetCollisionFilter } )
	beam[1].isSensor = true
	screenGroup:insert( beam[1] )
	beam[2] = display.newImage( myImageSheet , sheetInfo:getFrameIndex("beam3"))
	beam[2].x = 760
	beam[2].y = 0
	physics.addBody( beam[2], dynamic, { density=1, friction=0, bounce=15.0, radius=beam[2].radius, filter=jetCollisionFilter } )
	beam[2].isSensor = true
	beam[2].name = "beam"
	screenGroup:insert( beam[2] )
	beam[3] = display.newImage( myImageSheet , sheetInfo:getFrameIndex("beam"))
	beam[3].x = 760
	beam[3].y = 330
	physics.addBody( beam[3], dynamic, { density=1, friction=0, bounce=15.0, radius=beam[3].radius, filter=jetCollisionFilter } )
	beam[3].isSensor = true
	beam[3].name = "beam"
	screenGroup:insert( beam[3] )
	beam[4] = display.newImage( myImageSheet , sheetInfo:getFrameIndex("beam3"))
	beam[4].x = 760
	beam[4].y = 0
	...
	beam[5] = display.newImage( myImageSheet , sheetInfo:getFrameIndex("beam2"))
	beam[5].x = 760
	beam[5].y = 330
	...
	beam[6] = display.newImage( myImageSheet , sheetInfo:getFrameIndex("beam"))
	beam[6].x = 1300
	beam[6].y = 200
...
return function()
	...
	--timerIndex[52] = timer.performWithDelay( 1, (event, q), 1 )
	tnt:newTimer(1, explodeAlien(event, q), 1)
		end
end
end
...
function exitLevel(event, q)
		...
		if (q == "reload") then
			...
			timer.performWithDelay(1000, function() storyboard.gotoScene( "reloadLevel", mission2 ) end, 1)
		else
			Runtime:removeEventListener( "collision", onCollision)
			Runtime:removeEventListener("touch", screenTouched)
			Particles.StopEmitter("E1")
...
function levelDefeat()
	...
	local endScore = display.newText("               Mission 2 Zing: " .. score, 35, 165, "Arial", 24)
...
space2.x = space2.x - (speed/3)
space3.x = space3.x - (speed/3)
space4.x = space4.x - (speed/3)

if(space2.x < -239) then
space2.x = 760
end

if(space3.x < -239) then
space3.x = 760
end

if(space4.x < -239) then
space4.x = 760
end
...
function gameStart()
	...
	tnt:newTimer(2000, beamTrigger(event, 1, 1), 1)
	...
	--tnt:newTimer(19000, createBeams, 1)
	tnt:newTimer(18000, updateAlien(1), 1)
	tnt:newTimer(18000, updateAlien(2), 1)
	tnt:newTimer(18000, updateAlien(3), 1)
	--tnt:newTimer(20000, beamTrigger(event, 1, 1), 1)
	tnt:newTimer(25000, ufoFlight, 1)
	tnt:newTimer(32000, bunnyFlight, 1)
	--tnt:newTimer(36000, updateWarpGate, 1)
	tnt:newTimer(35000, beamTrigger(event, 1, 1), 1)
	tnt:newTimer(41000, beamTrigger(event, 2, 0), 1)
	tnt:newTimer(46000, updateAlien(1), 1)
	tnt:newTimer(46000, updateAlien(2), 1)
	tnt:newTimer(46000, updateAlien(3), 1)
	--tnt:newTimer(43000, updateAlien(4), 1)
	--tnt:newTimer(43000, updateAlien(5), 1)
	tnt:newTimer(47000, ufoFlight, 1)
	tnt:newTimer(55000, updateWarpGate, 1)
	tnt:newTimer(60000, beamTrigger(event, 1, 0), 1)
	tnt:newTimer(65000, updateAlien(1), 1)
	tnt:newTimer(65000, updateAlien(2), 1)
	tnt:newTimer(67000, updateAlien(3), 1)
	tnt:newTimer(70000, beamTrigger(event, 3, 0), 1)
