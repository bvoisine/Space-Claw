...
function onCollision( event )
        ...
        if ( event.phase == "began" ) then
				...
				timerIndex[40] = timer.performWithDelay( 1, explodeJet(), 1)	
				end
...
--timer.resume(timerIndex[18])
for t=1,9 do
if (timerIndex[t] ~= nil) then
timer.resume(timerIndex[t])
end
end
end
...
function explodeUFO(event)
				...
				Emitter1.x = ufo.x
				Emitter1.y = ufo.y
				timerIndex[43] = timer.performWithDelay(1, function() ufo.y = ufo.y + 5 end, 1)
				Particles.Update()
				else
				print "all done!!!!!!!!!!!!!!!!!!"
...
function jetFlight()
--print ("value of q: ", q)
--print ("jet value: ", jet[q].x)
--print ("double check y: " .. jet[j].y)
if (gameIsActive == true) then
	--if jet[1] == nil then
		--print "no jet available 2"
		--return
	--elseif (jet[1].x > -239) then
		--print ("Jet Flight:" .. jet[j].id .. " " .. jet[j].x)
		--jet[1]:transition(jet[1].x - (speed/.8))
		k = transition.to(jet[1], { time = 4000, x=760-1000})
		--print (jet[j].x)
	--else
	--jet[1].x = 760
	...
	--timerIndex[22] = timer.performWithDelay(1, jetFlight(1), 1)
...
print ("Warp Gate: ", warpNum, " : ", warpGateZing.x)
if (gameIsActive == true) then
...
function gameStart()
	...
	--timerIndex[1] = timer.performWithDelay(10000, updateWarpGate, 1)
	--timerIndex[2] = timer.performWithDelay(12000, mouseRun, 1)
	--timerIndex[5] = timer.performWithDelay(13000, updateWarpGate, 1)
	--timerIndex[3] = timer.performWithDelay(15000, ufoFlight, 1)
	timerIndex[4] = timer.performWithDelay(6000, updateJet, 1)
	--timerIndex[5] = timer.performWithDelay(10000, updateJet, 1)
