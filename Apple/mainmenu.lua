...
function scene:enterScene( event )
	...
	textMessage = display.newText("FREE", 10, 60, "Helvetica", 25 )
	textMessage:setTextColor( 255, 255, 255 )
	...
	local onPlayTouch = function( event )
		if event.phase == "release" then
			...
			Menu:removeSelf()
			Menu = nil
			storyboard.gotoScene( "levels", "fade", 300  )
	...
	btnAnim = transition.to( playBtn, { time=500, y=180, transition=easing.inOutExpo } )
	...
	btnAnim = transition.to( optBtn, { time=500, y=240, transition=easing.inOutExpo } )
	...
	btnAnim = transition.to( tutBtn, { time=500, y=290, transition=easing.inOutExpo } )
